...
class AuthService:
    ...
    async def login(self, db: DatabaseManager, login_name: str, password: str, login_type: int = 1) -> Optional[Dict[str, Any]]:
        """用户登录
        ...
        """
        return await self.user_service.login_with_password(db, login_name, password, login_type)
